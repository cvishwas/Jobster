				...
				<c:forEach items="${list}" var="job">
					<div class="">
						<a class="list-group-item list-group-item-action"
							href="#list-item${job.id}">${job.id} </a>
					</div>
				</c:forEach>
			</div>
			<!-- Post details  -->
			<div data-spy="scroll" data-target="#navbarEx" data-offset="0"
				class="scrollspy-example col-9">
				<c:forEach items="${list}" var="job">
					<div class="card mb-4">
									...
									<form name=""
										...
										<c:choose>
											<c:when test="${job.isLikedJobPost}">
												<input type="submit"
													class="font-weight-bold ml-4 bg-light border-0 text-primary text-"
													style="font-size: 30px;" name="favorite"
													title="Add to favorites" value="&#x2606;" />
											</c:when>
											<c:otherwise>
												<input type="submit"
													class="font-weight-bold ml-4 bg-light border-0 "
													style="font-size: 30px;" name="favorite"
													title="Add to favorites" value="&#x2606;" />
											</c:otherwise>
										</c:choose>
						...
						<div class="card-body">
							<h5 class="font-italic">
								Job categories:
								<c:forEach items="${job.jobCategories}" varStatus="status"
									var="category">
										${category.name}
